...
export function createBlankReport(regionId: string, period: ReportPeriod, year: number = 2026): DamkarReport {
  ...
  return {
    ...
    pejabat: {
      nama: reg.kadisDefault.nama,
      nip: reg.kadisDefault.nip,
      jabatan: reg.kadisDefault.jabatan
    },
    bagianA: {
...
export const INITIAL_REPORTS: DamkarReport[] = [
  ...REGIONS_KALTIM.map(r => createBlankReport(r.id, 'SEMESTER_1', 2026)),
  ...REGIONS_KALTIM.map(r => createBlankReport(r.id, 'SEMESTER_2', 2026))
];
